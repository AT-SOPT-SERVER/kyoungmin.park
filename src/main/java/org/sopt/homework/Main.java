package org.sopt.homework;

import java.util.List;
import java.util.Scanner;

import org.sopt.homework.controller.PostController;
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			switch (input) {
				...
				case "2":
					System.out.println("\n📚 [전체 게시글 조회]");
					for (PostDto postDto : controller.getAllPosts()) {
						System.out.printf("🆔 %d | 📌 제목: %s\n", postDto.getId(), postDto.getPost().getTitle());
					}
					break;
				...
				case "3":
					...
					PostDto found = controller.getPostById(id);
					if (found != null) {
						System.out.println("📄 게시글 상세 내용:");
						System.out.println("-------------------------------------");
						System.out.printf("🆔 ID: %d\n", found.getId());
						System.out.printf("📌 제목: %s\n", found.getPost().getTitle());
						System.out.println("-------------------------------------");
					} else {
				...
				case "6":
					...
					List<PostDto> results = controller.searchPostsByKeyword(keyword);
					if (results.isEmpty()) {
						System.out.println("🔍 검색 결과가 없습니다.");
					} else {
						System.out.println("📋 검색 결과:");
						for (PostDto postDto : results) {
							System.out.printf("🆔 %d | 📌 제목: %s\n", postDto.getId(), postDto.getPost().getTitle());
						}
					}
					break;
